...
public class Assembler {
                ...
                try {
                    ...
                    if (statement.label() != null) {
                        if (_symbolTable.containsKey(statement.label())) {
                            ...
                            _symbolTable.put(statement.label(), new Integer(_locctr));
                        }
                    }
        ...
        try (FileInputStream istream = new FileInputStream(input);
            ...
            while (istream.available()>0) {
                ...
                if (statement.compareTo("START") == 0) {
                    ...
                    System.out.println(statement + "\t\t" + objectCode);
                    objectProgram.write(new DataRecord(statement.operand1(),_symbolTable.get(statement.operand1())).toObjectProgram() + '\n');
                }
                else if (statement.compareTo("EXTREF") == 0) {
                    System.out.println(statement + "\t\t" + objectCode);
                    if(statement.operand2()!=null)
                    {
                        objectProgram.write(new RefRecord(statement.operand1(),statement.operand2()).toObjectProgram() + '\n');
                    }
                    else
                    {
                        objectProgram.write(new RefRecord(statement.operand1()).toObjectProgram() + '\n');
                    }
                }    
    ...
    private String assembleInstruction(Statement statement) {
        ...
        if (_opTable.containsKey(statement.operation())) {
            ...
            if(statement.operand2()==null){
            int code1 = Integer.parseInt(statement.operand1());
            objCode =  String.format("%06X",code1);
            }
            else{
                int code1 = Integer.parseInt(statement.operand1());
                objCode =  String.format("%06X",code1);
                int code2 = Integer.parseInt(statement.operand2());
                objCode += String.format("^%06X",code2);
            }
        } 
